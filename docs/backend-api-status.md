...
AD/mock authentikáció
csoporttagságok lekérése
belső role feloldása
felhasználó keresése ad_username alapján
új user létrehozása vagy meglévő frissítése
role_id, last_login_at, role_synced_at mezők frissítése
...
Az access token rövid életű, jelenleg konfigurációból olvasott értékkel működik. A refresh token hosszabb életű session kezelésre szolgál.
...
a kliens elküldi a refresh tokent
a backend hash alapján megkeresi az aktív refresh tokent
ellenőrzi, hogy nem járt-e le és nincs-e visszavonva
új access tokent generál
új refresh tokent generál
a régi refresh tokent visszavonja
audit eseményt ír
...
Ez csökkenti annak kockázatát, hogy egy korábban megszerzett refresh token hosszabb ideig használható maradjon.
...
Ez azért fontos, mert így a kliens alapból nem kapja meg az összes érzékeny adatot, csak akkor, amikor a felhasználó ténylegesen kéri.
...
credential aktív állapotának ellenőrzése
titkosított mezők ellenőrzése
AES-GCM visszafejtés
last_accessed_at frissítése
audit esemény írása
plaintext érték visszaadása
...
A hash-chain működését MySQL lekérdezésekkel is ellenőriztük. Az egymást követő audit rekordoknál az új rekord previous_hash mezője az előző rekord hash értékére mutat.
...
felhasználói bind végrehajtása
user keresése sAMAccountName alapján
display name, email és group adatok lekérése
AD csoportnév kinyerése memberOf mezőből
...
Még hátralévő API feladatok

A backend API jelenlegi készültsége MVP szinten körülbelül 92%.

Hátralévő fontosabb feladatok:

egységes DTO validáció és hibaválaszok
nem-admin szerepkörök további integrációs tesztelése
LDAP éles környezetes kipróbálása
audit hash-chain validáló belső eszköz vagy endpoint
API dokumentáció bővítése
szakdolgozati leírás véglegesítése
opcionális jelszógenerátor endpoint
opcionális konfigurációs ellenőrző endpoint fejlesztői/admin célra